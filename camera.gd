...
func _process(delta):
	...
	if Input.is_action_pressed("ui_camera_move_forward"):
		movement = position.direction_to(target.position) * delta * CAMERA_MOVEMENT_SPEED;
	if Input.is_action_pressed("ui_camera_move_backward"):
		movement = position.direction_to(target.position) * delta * CAMERA_MOVEMENT_SPEED * -1;
